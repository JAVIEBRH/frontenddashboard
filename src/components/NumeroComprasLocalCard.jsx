...
const NumeroComprasLocalCard = ({ 
  title = 'Número de Compras', 
  subtitle = 'Total de transacciones realizadas en el local'
}) => {
  const theme = useTheme();
  const [loading, setLoading] = useState(true);
  const [data, setData] = useState({
    numeroCompras: 0,
    numeroComprasMesAnterior: 0,
    porcentajeCambio: 0,
    esPositivo: true,
    tendenciaMensual: []
  });
    ...
    const fetchData = async () => {
      try {
        ...
        if (result.status === 'success' && result.data) {
          ...
          let numeroCompras = 0;
          let numeroComprasMesAnterior = 0;
          ...
          result.data.forEach(venta => {
            ...
            if (fechaVenta.getMonth() === (mesActual - 1) && fechaVenta.getFullYear() === anioActual) {
              numeroComprasMesAnterior++;
            }
          });

          const porcentajeCambio = numeroComprasMesAnterior > 0 
            ? ((numeroCompras - numeroComprasMesAnterior) / numeroComprasMesAnterior) * 100 
            : 0;

          // Generar tendencia mensual simulada
          const tendenciaMensual = [
            { mes: 'Ene', compras: numeroCompras * 0.8 },
            { mes: 'Feb', compras: numeroCompras * 0.85 },
            { mes: 'Mar', compras: numeroCompras * 0.9 },
            { mes: 'Abr', compras: numeroCompras * 0.92 },
            { mes: 'May', compras: numeroCompras * 0.95 },
            { mes: 'Jun', compras: numeroCompras * 0.98 },
            { mes: 'Jul', compras: numeroCompras }
          ];
        ...
        console.error('Error obteniendo datos de número de compras:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const generarPuntosGrafico = () => {
    ...
    const puntos = data.tendenciaMensual.map((mes, index) => {
      ...
      const maxCompras = Math.max(...data.tendenciaMensual.map(m => m.compras));
      const y = maxCompras > 0 ? 40 - (mes.compras / maxCompras) * 30 : 30;
      return `${x} ${y}`;
    });
  ...
  const tooltipText = `Número de compras locales:
Este mes: ${formatValue(data.numeroCompras)} compras
Mes anterior: ${formatValue(data.numeroComprasMesAnterior)} compras
    ...
    >
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 2 }}>
        <Box sx={{ flex: 1 }}>
          ...
          >
            {formatValue(data.numeroCompras)}
          </Typography>
          <Typography 
            variant="body2" 
            sx={{ 
...
export default NumeroComprasLocalCard;

...
const UtilidadesCard = ({
  ...
  const generarPuntosGrafico = () => {
    if (
      !utilidadesData.tendencia_mensual ||
      utilidadesData.tendencia_mensual.length === 0
    ) {
    ...
    const puntos = utilidadesData.tendencia_mensual.map((mes, index) => {
      ...
      const maxUtilidades = Math.max(
        ...utilidadesData.tendencia_mensual.map((m) => m.utilidades)
      );
      const y =
        maxUtilidades > 0 ? 40 - (mes.utilidades / maxUtilidades) * 30 : 30;
      return `${x} ${y}`;
    });
  ...
  const totalUtilidades = utilidadesData.utilidades_mes_actual;
  const porcentajeLocal =
    totalUtilidades > 0 ? (utilidadesLocal / totalUtilidades) * 100 : 0;
  const porcentajeDelivery =
    totalUtilidades > 0 ? (utilidadesDelivery / totalUtilidades) * 100 : 0;
...
${utilidadesData.es_positivo ? "📈" : "📉"} Variación: ${
    utilidadesData.es_positivo ? "+" : ""
  }${utilidadesData.porcentaje_cambio.toFixed(1)}%

🏪 DESGLOSE POR ORIGEN:
   🏪 Utilidades Local: ${formatValue(
     utilidadesLocal
   )} (${porcentajeLocal.toFixed(1)}%)
   🚚 Utilidades Delivery: ${formatValue(
     utilidadesDelivery

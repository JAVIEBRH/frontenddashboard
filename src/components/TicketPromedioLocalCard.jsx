...
const TicketPromedioLocalCard = ({ 
  title = 'Ticket Promedio', 
  subtitle = 'Valor promedio por venta en el local'
}) => {
  const theme = useTheme();
  const [loading, setLoading] = useState(true);
  const [data, setData] = useState({
    ticketPromedio: 0,
    ticketPromedioMesAnterior: 0,
    porcentajeCambio: 0,
    esPositivo: true,
    tendenciaMensual: []
  });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (result.status === 'success' && result.data) {
          ...
          result.data.forEach(venta => {
            ...
            if (fechaVenta.getMonth() === mesActual && fechaVenta.getFullYear() === anioActual) {
              ventasMesActual.push(precio);
            }
            
            // Ventas del mes anterior
            if (fechaVenta.getMonth() === (mesActual - 1) && fechaVenta.getFullYear() === anioActual) {
              ventasMesAnterior.push(precio);
            }
          });

          const ticketPromedio = ventasMesActual.length > 0 
            ? ventasMesActual.reduce((sum, precio) => sum + precio, 0) / ventasMesActual.length 
            : 0;
          
          const ticketPromedioMesAnterior = ventasMesAnterior.length > 0 
            ? ventasMesAnterior.reduce((sum, precio) => sum + precio, 0) / ventasMesAnterior.length 
            : 0;

          const porcentajeCambio = ticketPromedioMesAnterior > 0 
            ? ((ticketPromedio - ticketPromedioMesAnterior) / ticketPromedioMesAnterior) * 100 
            : 0;

          // Generar tendencia mensual simulada
          const tendenciaMensual = [
            { mes: 'Ene', ticket: ticketPromedio * 0.8 },
            { mes: 'Feb', ticket: ticketPromedio * 0.85 },
            { mes: 'Mar', ticket: ticketPromedio * 0.9 },
            { mes: 'Abr', ticket: ticketPromedio * 0.92 },
            { mes: 'May', ticket: ticketPromedio * 0.95 },
            { mes: 'Jun', ticket: ticketPromedio * 0.98 },
            { mes: 'Jul', ticket: ticketPromedio }
          ];
        ...
        console.error('Error obteniendo datos de ticket promedio:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const generarPuntosGrafico = () => {
    ...
    const puntos = data.tendenciaMensual.map((mes, index) => {
      ...
      const maxTicket = Math.max(...data.tendenciaMensual.map(m => m.ticket));
      const y = maxTicket > 0 ? 40 - (mes.ticket / maxTicket) * 30 : 30;
      return `${x} ${y}`;
    });
  ...
  const tooltipText = `Ticket promedio local:
Este mes: ${formatValue(data.ticketPromedio)}
Mes anterior: ${formatValue(data.ticketPromedioMesAnterior)}
    ...
    >
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 2 }}>
        <Box sx={{ flex: 1 }}>
          ...
          >
            {formatValue(data.ticketPromedio)}
          </Typography>
          <Typography 
            variant="body2" 
            sx={{ 
...
export default TicketPromedioLocalCard;

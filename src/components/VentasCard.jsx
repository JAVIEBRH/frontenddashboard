...
const VentasCard = ({
  ...
  const generarPuntosGrafico = () => {
    ...
    const puntos = ventasData.tendencia_mensual.map((mes, index) => {
      ...
      const maxVentas = Math.max(
        ...ventasData.tendencia_mensual.map((m) => m.ventas)
      );
  ...
  const totalVentas = ventasData.ventas_totales;
...
📊 Total de pedidos: ${
    totalPedidos > 0 ? totalPedidos.toLocaleString("es-CL") : "N/A"
  }

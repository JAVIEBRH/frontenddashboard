...
export default function Clientes({ refreshTrigger = 0 }) {
  ...
  const generarClientesEnRiesgoData = (clientesData, pedidosData) => {
    ...
    const clientesConTicketPromedio = clientesEnRiesgoReales.map((cliente) => {
      ...
      const pedidosCliente = pedidosData.filter((p) => {
        ...
        const matchEmail =
          emailCliente && emailPedido && emailCliente === emailPedido;
      ...
      const ticketPromedio =
        pedidosCliente.length > 0 ? totalComprado / pedidosCliente.length : 0;

      // Calcular días sin comprar
      const fechaUltimo = parseFecha(cliente.ultimo_pedido || "");
      const diasSinComprar = fechaUltimo
        ? Math.floor((hoy - fechaUltimo) / (1000 * 60 * 60 * 24))
        : 0;
    ...
    return clientesConTicketPromedio.sort(
      (a, b) => b.diasSinComprar - a.diasSinComprar
    );
  };
  ...
  const generarClientesVipData = (clientesVIPyFrecuenciaData, pedidosData) => {
    if (
      !clientesVIPyFrecuenciaData ||
      clientesVIPyFrecuenciaData.length === 0 ||
      !pedidosData ||
      pedidosData.length === 0
    ) {
      return [];
    ...
    const clientesVipReales = clientesVIPyFrecuenciaData.map((cliente) => {
      ...
      const fechaUltimo = parseFecha(cliente.ultimo_pedido || "");
      const diasDesdeUltimaCompra = fechaUltimo
        ? Math.floor((hoy - fechaUltimo) / (1000 * 60 * 60 * 24))
        : 0;
    ...
    return clientesVipReales.sort((a, b) => b.totalComprado - a.totalComprado);
  };
    ...
    return (
      clientesConEstado
        .filter((cliente) => {
          ...
          const matchesTipo =
            filterTipo === "Todos" || cliente.tipo === filterTipo;
    ...
    try {
      ...
      const clientesData = (clientesDataRaw || []).map((c, idx) => {
        ...
        const ultimaFechaPedido =
          fechasPedidos.length > 0 ? fechasPedidos[0] : parseFecha(c.fecha);
        ...
        const nombreMostrar =
          c.nombre && c.nombre.trim() !== "" && !c.nombre.includes("Cliente")
            ? c.nombre.trim()
        ...
        return {
          ...
          estado: calcularEstadoCliente(
            ultimaFechaPedido
              ? ultimaFechaPedido.toISOString().split("T")[0]
              : c.fecha
          ),
          ...
          ultimo_pedido: ultimaFechaPedido
            ? ultimaFechaPedido.toISOString().split("T")[0]
            : c.fecha || "",
      ...
      const inactivos = clientesData.filter(
        (c) => c.estado === "Inactivo"
      ).length;
  ...
  const calcularListas = (clientesData, pedidosData) => {
    ...
    pedidosData.forEach((pedido) => {
      ...
      if (
        !comprasPorClave[clave].ultimo ||
        new Date(pedido.fecha) > new Date(comprasPorClave[clave].ultimo)
      ) {
    ...
    clientesData.forEach((cliente) => {
      ...
      if (!clientesPorClave[clave]) {
        ...
        if (
          fechaNueva &&
          (!fechaExistente || new Date(fechaNueva) > new Date(fechaExistente))
        ) {
    ...
    const clientesEnriquecidos = Object.values(clientesPorClave).map(
      (cliente) => {
        const clave = claveAgrupacion(cliente.direccion);
        const compras = comprasPorClave[clave] || {
          total: 0,
          frecuencia: 0,
          ultimo: null,
          totalBidones: 0,
        ...
        const bidonesPorPedido =
          compras.frecuencia > 0 && compras.totalBidones > 0
        ...
        const fechaFinal =
          fechaCompras && fechaCliente
            ? new Date(fechaCompras) > new Date(fechaCliente)
              ? fechaCompras
              : fechaCliente
            : fechaCompras || fechaCliente;
    ...
    const clientesConBidones = topVIP.filter(
      (c) => c.bidonesPorPedido && c.bidonesPorPedido > 0
    );
    ...
    const direccionesVIPTemp = new Set(
      topVIP.map((c) => (c.direccion || "").toLowerCase())
    );
    const direccionesFrecuenciaTemp = new Set(
      topFrecuencia.map((c) => (c.direccion || "").toLowerCase())
    );
    const clientesVIPyFrecuenciaTemp = topVIP.filter((c) =>
      direccionesFrecuenciaTemp.has((c.direccion || "").toLowerCase())
    );
    ...
    const vipData = generarClientesVipData(
      clientesVIPyFrecuenciaTemp,
      pedidosData
    );
  ...
  const filteredFrecuencia = React.useMemo(() => {
    ...
    return clientesFrecuencia.filter(
      (cliente) =>
        (filtroEstadoFrecuencia === "Todos" ||
          cliente.estado === filtroEstadoFrecuencia) &&
        ((cliente.nombre || "").toLowerCase().includes(searchLower) ||
          (cliente.email || "").toLowerCase().includes(searchLower) ||
          (cliente.direccion || "").toLowerCase().includes(searchLower))
    );
  ...
  const direccionesVIP = new Set(
    (clientesVIP || []).map((c) => (c.direccion || "").toLowerCase())
  );
  const direccionesFrecuencia = new Set(
    (clientesFrecuencia || []).map((c) => (c.direccion || "").toLowerCase())
  );
  const clientesVIPyFrecuencia = (clientesVIP || []).filter((c) =>
    direccionesFrecuencia.has((c.direccion || "").toLowerCase())
  );
  const cantidadVIPyFrecuencia = clientesVIPyFrecuencia.length;
  const tooltipVIPyFrecuencia = clientesVIPyFrecuencia
    .map((c) => c.direccion || "Sin dirección")
    .join("\n");
  ...
  const clientesReactivados = React.useMemo(() => {
    ...
    pedidos.forEach((pedido) => {
      const dirNorm = (pedido.dire || pedido.direccion || "")
        .trim()
        .toLowerCase()
        .replace(/\s+/g, " ");
      if (!dirNorm) return;
    ...
    Object.values(pedidosPorDireccion).forEach((listaPedidos) => {
      ...
      for (let i = 1; i < listaPedidos.length; i++) {
        ...
        if (fechaPrev && fechaActual) {
          const diff =
            (fechaActual.getTime() - fechaPrev.getTime()) /
            (1000 * 60 * 60 * 24);
          if (diff > 75) {
                  ...
                  <TableCell>
                    <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                      <Avatar sx={{ bgcolor: "#3b82f6" }}>
                        {(cliente.direccion || cliente.nombre || "?")
                          .charAt(0)
                          .toUpperCase()}
                      </Avatar>
                      <Box>
                        ...
                        >
                          {cliente.direccion && cliente.direccion.trim() !== ""
                            ...
                            : cliente.nombre &&
                              cliente.nombre.trim() !== "" &&
                              !cliente.nombre.startsWith("Cliente ")
                            ? cliente.nombre
  ...
  const totalClientesActivos = clientesConEstado.filter(
    (c) => c.estado === "Activo"
  ).length;
  const totalClientesVIP = clientes.filter((c) => c.total_comprado > 0).length;
  const totalVentas = clientes.reduce(
    (sum, c) => sum + (c.total_comprado || 0),
    0
  );
  // Calcular churn (clientes inactivos) - usar clientesConEstado para consistencia
  const churnAbs = clientesConEstado.filter(
    (c) => c.estado === "Inactivo"
  ).length;
  const churnPct =
    totalClientes > 0 ? Math.round((churnAbs / totalClientes) * 100) : 0;
  ...
  const topCrecimiento = (() => {
    ...
    const pedidosPorDireccion = {};
    pedidos.forEach((p) => {
      const dirNorm = (p.dire || p.direccion || "")
        .trim()
        .toLowerCase()
        .replace(/\s+/g, " ");
      if (!dirNorm) return;
    ...
    const crecimientoPorDireccion = Object.entries(pedidosPorDireccion).map(
      ([dir, lista]) => {
        let montoPeriodoActual = 0,
          montoPeriodoAnterior = 0;
        let pedidosPeriodoActual = 0,
        ...
        const crecimiento =
          montoPeriodoAnterior === 0
            ? montoPeriodoActual
            : montoPeriodoActual - montoPeriodoAnterior;
        const porcentajeCrecimiento =
          montoPeriodoAnterior === 0
            ? montoPeriodoActual > 0
              ? 100
              : 0
            : ((montoPeriodoActual - montoPeriodoAnterior) /
        ...
        return {
          ...
          promedioPeriodoActual:
            pedidosPeriodoActual > 0
              ? montoPeriodoActual / pedidosPeriodoActual
              : 0,
          promedioPeriodoAnterior:
            pedidosPeriodoAnterior > 0
              ? montoPeriodoAnterior / pedidosPeriodoAnterior
              : 0,
          nombrePeriodoActual,
          nombrePeriodoAnterior,
        };
  ...
  const generarNotificaciones = () => {
    ...
    clientesEnRiesgo.forEach((cliente) => {
      notificaciones.push({
        ...
        } no ha comprado en ${Math.floor(
          (new Date() - parseFecha(cliente.ultimo_pedido)) /
            (1000 * 60 * 60 * 24)
        )} días`,
        cliente: cliente,
        timestamp: new Date(),
    ...
    clientesInactivos.slice(0, 10).forEach((cliente) => {
      const fechaUltimo = parseFecha(cliente.ultimo_pedido);
      const diasSinComprar = fechaUltimo
        ? Math.floor((new Date() - fechaUltimo) / (1000 * 60 * 60 * 24))
        : 0;
      const fechaFormateada = fechaUltimo
      ...
      notificaciones.push({
        ...
        mensaje: `${
          cliente.nombre || cliente.direccion || "Cliente sin nombre"
        } - Último pedido: ${fechaFormateada} (${diasSinComprar} días)`,
        cliente: cliente,
        timestamp: new Date(),
      });
    });
    ...
    const tasaActividad =
      (clientesConEstado.filter((c) => c.estado === "Activo").length /
        clientesConEstado.length) *
      100;
    if (tasaActividad < 50) {
      notificaciones.push({
    ...
    const clientesVIPInactivos = (clientesVIP || []).filter(
      (c) => c.estado === "Inactivo"
    );
    ...
    if (
      ...
      clientesConEstado.filter((c) => c.estado === "Activo").length * 0.3
    ) {
      notificaciones.push({
    ...
    if (topCrecimiento.length > 0) {
      const clientesConCrecimiento = topCrecimiento.filter(
        (c) => c.porcentajeCrecimiento > 50
      ).length;
              ...
              <Table stickyHeader>
                ...
                <TableBody>
                  {(clientesEnRiesgoData || [])
                    .sort(
                      (a, b) =>
                        (b.diasSinComprar || 0) - (a.diasSinComprar || 0)
                    )
                    .map((cliente) => (
                      <TableRow
                        key={cliente.id}
                      ...
                      >
                        <TableCell>
                          <Tooltip
                            title={
                              cliente.estado === "critico"
                                ? "Cliente en riesgo crítico"
                                : "Cliente en riesgo moderado"
                            }
                            arrow
                          >
                            <Box
                        ...
                        <TableCell>
                          <Tooltip
                            title={`${cliente.direccion} - ${cliente.email}`}
                            arrow
                          >
                            <Box>
                              <Typography
                        ...
                        <TableCell>
                          <Tooltip
                            title={`Última compra: ${cliente.ultimaCompra}`}
                            arrow
                          >
                            <Typography
                              variant="body2"
                        ...
                        <TableCell>
                          <Tooltip
                            title={`Ticket promedio: ${formatTicketPromedio(
            ...
            <TableContainer sx={{ maxHeight: "60vh" }}>
              <Table stickyHeader>
                ...
                <TableBody>
                  {(clientesVipData || [])
                    .sort(
                      (a, b) => (b.totalComprado || 0) - (a.totalComprado || 0)
                    )
                    .map((cliente) => (
                      <TableRow
                        key={cliente.id}
                      ...
                      >
                        ...
                        <TableCell>
                          <Tooltip
                            title={`Total comprado: ${formatTicketPromedio(
                        ...
                        <TableCell>
                          <Tooltip
                            title={`Número de pedidos: ${cliente.pedidos}`}
                            arrow
                          >
                            <Typography
                              variant="body2"
          ...
          {error && (
            ...
            >
              ...
              <Typography variant="body2" sx={{ mt: 1 }}>
                Por favor, intenta recargar la página o contacta al
            ...
            <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
              ...
              <Tooltip title="Notificaciones y Alertas" placement="bottom">
                <IconButton
                  ...
                  sx={{
                    bgcolor:
                      notificacionesNoLeidas > 0 ? "#fef3c7" : "transparent",
                    "&:hover": {
                      bgcolor:
                        notificacionesNoLeidas > 0 ? "#fde68a" : "#f1f5f9",
                    },
                    borderRadius: 2,
                    p: 1.5,
                  }}
                  ...
                  >
                    {notificacionesNoLeidas > 0 ? (
                      <NotificationsActiveIcon
                        sx={{ color: theme.palette.warning.dark, fontSize: 24 }}
                      />
                    ) : (
                      <NotificationsIcon
                        sx={{
                ...
                <Box sx={{ p: 3 }}>
                  ...
                  {notificaciones.length === 0 ? (
                    ...
                    <Box sx={{ maxHeight: 500, overflow: "auto" }}>
                      ...
                      {(() => {
                        const errores = notificaciones.filter(
                          (n) => n.tipo === "error"
                        );
                        const warnings = notificaciones.filter(
                          (n) => n.tipo === "warning"
                        );
                        const info = notificaciones.filter(
                          (n) => n.tipo === "info"
                        );
          ...
          <Grid container columns={12} spacing={3} sx={{ mb: 4 }}>
            <Grid span={3}>
              ...
              >
                <CardContent sx={{ p: 3 }}>
                  <Tooltip
                    title="Cantidad total de clientes únicos registrados en el sistema."
                    placement="top"
                    arrow
                  >
                    <Typography
            ...
            <Grid span={3}>
              <Card
                sx={{
                  ...
                  "@keyframes glowWave": {
                    "0%": {
                      transform:
                        "translateX(-100%) translateY(-100%) rotate(0deg)",
                    },
                    "50%": {
                      transform:
                        "translateX(100%) translateY(100%) rotate(180deg)",
                    },
                    "100%": {
                      transform:
                        "translateX(-100%) translateY(-100%) rotate(360deg)",
                    },
                  },
                  "@keyframes glowPulse": {
                    "0%, 100%": { opacity: 0.7 },
              ...
              >
                <CardContent sx={{ p: 3, position: "relative", zIndex: 2 }}>
                  <Tooltip
                    title="Clientes que han realizado al menos un pedido en los últimos 75 días."
                    placement="top"
                    arrow
                  >
                    <Typography
            ...
            <Grid span={3}>
              ...
              >
                <CardContent sx={{ p: 3 }}>
                  <Tooltip
                    title="Clientes que volvieron a comprar después de más de 75 días sin hacerlo."
                    placement="top"
                    arrow
                  >
                    <Typography
            ...
            <Grid span={3}>
              <Box sx={{ position: "relative" }}>
                <Card
                  sx={{
                    bgcolor: showVipTable
                      ? theme.palette.primary.main
                      : "background.paper",
                    boxShadow: showVipTable
                      ? theme.shadows[4]
                      : theme.shadows[1],
                    borderRadius: 3,
                    border: `1px solid ${
                ...
                >
                  <CardContent sx={{ p: 3, position: "relative" }}>
                    <Tooltip
                      title="Clientes que están tanto en el top 15 de mayor dinero aportado como en el top 15 de mayor frecuencia de compra. Click para ver detalles."
                      placement="top"
                      arrow
                    >
                      <Typography
            ...
            <Grid span={3}>
              ...
              >
                <CardContent sx={{ p: 3, position: "relative", zIndex: 2 }}>
                  <Tooltip
                    title="Clientes que llevan más de 75 días sin comprar (churn absoluto y porcentaje)."
                    placement="top"
                    arrow
                  >
                    <Typography
            ...
            <Grid span={3}>
              ...
              >
                <CardContent sx={{ p: 3, position: "relative", zIndex: 2 }}>
                  <Tooltip
                    title="Clientes cuya primera compra fue en los últimos 75 días."
                    placement="top"
                    arrow
                  >
                    <Typography
            ...
            <Grid span={3}>
              <Box sx={{ position: "relative" }}>
                <Card
                  sx={{
                    bgcolor: showRiesgoTable
                      ? theme.palette.primary.main
                      : "background.paper",
                    boxShadow: showRiesgoTable
                      ? theme.shadows[4]
                      : theme.shadows[1],
                    borderRadius: 3,
                    border: `1px solid ${
                ...
                >
                  <CardContent sx={{ p: 3, position: "relative" }}>
                    <Tooltip
                      title="Clientes que llevan entre 60 y 75 días sin comprar (cerca de volverse inactivos). Click para ver detalles."
                      placement="top"
                      arrow
                    >
                      <Typography
                  ...
                  {clientesPagina.map((cliente) => (
                    ...
                    >
                      <TableCell>
                        ...
                        >
                          <Avatar sx={{ bgcolor: "#3b82f6" }}>
                            {cliente.nombre && cliente.nombre.length > 0
            ...
            >
              ...
              {(() => {
                ...
                if (startPage > 1) {
                  ...
                  if (startPage > 2) {
                    pages.push(
                      <Typography key="ellipsis1" sx={{ px: 0.5 }}>
                        ...
                      </Typography>
                    );
                  }
                ...
                if (endPage < totalPaginas) {
                  if (endPage < totalPaginas - 1) {
                    pages.push(
                      <Typography key="ellipsis2" sx={{ px: 0.5 }}>
                        ...
                      </Typography>
                    );
                  }
                  pages.push(
                    <Button
                      ...
                      variant={
                        pagina === totalPaginas ? "contained" : "outlined"
                      }
                      size="small"
                      sx={{ minWidth: 32, px: 1 }}
                    >
                    ...
                    >
                      ...
                      >
                        {topCrecimiento.length > 0
                          ? topCrecimiento[0].nombrePeriodoActual
                          : "Período Actual"}
                      </TableCell>
                      <TableCell
                        sx={{
                      ...
                      >
                        {topCrecimiento.length > 0
                          ? topCrecimiento[0].nombrePeriodoAnterior
                          : "Período Anterior"}
                      </TableCell>
                      <TableCell
                        sx={{
                    ...
                    {topCrecimiento.map((c, i) => (
                      <TableRow key={i}>
                        ...
                        <TableCell>
                          <Chip
                            label={`${
                              c.porcentajeCrecimiento >= 0 ? "+" : ""
                            }${c.porcentajeCrecimiento.toFixed(1)}%`}
                            size="small"
                            sx={{
                              bgcolor:
                                c.porcentajeCrecimiento >= 0

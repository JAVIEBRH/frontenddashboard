...
import {
  getFactoresPrediccion,
  getPredictorInteligente,
  getTrackingMetricas,
  getTrackingReporte,
  registrarPedidosReales,
  getUltimasPredicciones,
  getPedidos,
  getVentasHistoricas,
} from "../services/api";
...
export default function Predictor() {
  ...
  const [pedidosHistoricosCompletos, setPedidosHistoricosCompletos] = useState(
    []
  ); // Lista completa de pedidos individuales
  const [rangoDatos, setRangoDatos] = useState({
    fechaMinima: null,
    fechaMaxima: null,
    ultimaActualizacion: null,
  });
  const [metricasValidacion, setMetricasValidacion] = useState({
    mae: null,
    rmse: null,
    mape: null,
  });
  ...
  const registrarPedidosRealesHandler = async () => {
    ...
    try {
      await registrarPedidosReales(
        fechaPedidosReales,
        parseInt(pedidosRealesInput),
        prediccion.tipoCliente
      );
      ...
      mostrarNotificacion(
        "Error registrando pedidos reales. Por favor intenta nuevamente.",
        "error"
      );
    }
  };
  ...
  const normalizarDatos = (pedidos, ventasHistoricas) => {
    ...
    if (Array.isArray(pedidos)) {
      pedidos.forEach((p) => {
        const fechaStr =
          p.fecha || p.createdAt || p.deliveryDate || p.fecha_creacion;
        ...
        registro.pedidos += 1;
        registro.total += parseFloat(
          p.precio || p.price || p.total || p.monto || 0
        );
        registro.count += 1;
      });
    }
  ...
  const cargarDatosHistoricos = async () => {
    try {
      ...
      const pedidosCompletos = Array.isArray(pedidosData)
        ? pedidosData
            .map((p) => ({
              ...p,
              fechaNormalizada: parseFechaLocal(
                p.fecha || p.createdAt || p.deliveryDate || p.fecha_creacion
              ),
            }))
            .filter(
              (p) => p.fechaNormalizada && !isNaN(p.fechaNormalizada.getTime())
            )
        : [];
      ...
      if (datosNormalizados.length >= 30) {
        const resultadoBacktest = realizarBacktesting(
          datosNormalizados,
          "promedioMovil"
        );
  ...
  const calcularMAE = (predicciones, valoresReales) => {
    if (
      !predicciones ||
      !valoresReales ||
      predicciones.length !== valoresReales.length
    ) {
      return null;
    }
  ...
  const calcularRMSE = (predicciones, valoresReales) => {
    ...
    const erroresCuadrados = predicciones.map((p, i) =>
      Math.pow(p - valoresReales[i], 2)
    );
    const promedioErroresCuadrados =
      erroresCuadrados.reduce((sum, e) => sum + e, 0) / erroresCuadrados.length;
  ...
  const calcularMAPE = (predicciones, valoresReales) => {
    ...
    return (
      erroresPorcentuales.reduce((sum, e) => sum + e, 0) /
      erroresPorcentuales.length
    );
  };
  ...
  const modeloPromedioMovil = (datos, ventana = 7) => {
    ...
    return (
      ultimosDatos.reduce((sum, d) => sum + d.pedidos, 0) / ultimosDatos.length
    );
  };
  ...
  const modeloDiaSemana = (datos, fechaObjetivo) => {
    ...
    return (
      datosMismoDia.reduce((sum, d) => sum + d.pedidos, 0) /
      datosMismoDia.length
    );
  };
  ...
  const realizarBacktesting = (datosHistoricos, modelo = "promedioMovil") => {
    ...
    datosValidacion.forEach((dato, index) => {
      ...
      if (modelo === "promedioMovil") {
        ...
        prediccion =
          datosEntrenamiento.reduce((sum, d) => sum + d.pedidos, 0) /
  ...
  const calcularPrediccionLocal = (fechaObjetivo) => {
    ...
    if (ultimos14Dias.length >= 2) {
      ...
      const promedioX = (n - 1) / 2;
      const promedioY =
        ultimos14Dias.reduce((sum, d) => sum + d.pedidos, 0) / n;
    ...
    const desviacion = Math.sqrt(
      datosPorDia.reduce(
        (sum, d) => sum + Math.pow(d.pedidos - promedioGeneral, 2),
        0
      ) / datosPorDia.length
    );
    detalles.desviacion = desviacion;

    const rangoInferior = Math.max(
      1,
      Math.round(prediccionEnsemble - desviacion * 1.5)
    );
  ...
  const generarPrediccionInteligente = async () => {
    ...
    try {
      ...
      if (prediccionLocal) {
        ...
        mostrarNotificacion(
          "No hay suficientes datos históricos para calcular la predicción",
          "error"
        );
      }
    } catch (error) {
      ...
      mostrarNotificacion(
        "Error al calcular la predicción. Por favor intenta nuevamente.",
        "error"
      );
    } finally {
      setLoading(false);
  ...
  const generarAnalisisInteligente = (resultado, prediccion) => {
    ...
    const tipoClienteLabel =
      tipoClienteLabels[prediccion.tipoCliente] || prediccion.tipoCliente;

    let analisis = {
      titulo: `Predicción Inteligente - ${fecha.toLocaleDateString(
        "es-ES"
      )} - ${tipoClienteLabel}`,
      fecha: prediccion.fecha,
    ...
    analisis.resumen = `Se esperan ${resultado.prediccion} pedidos (rango: ${
      resultado.rango_confianza[0]
    }-${resultado.rango_confianza[1]}) para el ${
      diasSemana[diaSemana]
    } ${fecha.toLocaleDateString("es-ES")} con ${
    ...
    const amplitudRango =
      resultado.rango_confianza[1] - resultado.rango_confianza[0];
    if (amplitudRango <= 2) {
      detalles.push({
        tipo: "success",
    ...
    if (resultado.es_anomalia) {
      detalles.push({
        tipo: "error",
        mensaje:
          "⚠️ Día detectado como anómalo: Considerar factores especiales",
        icono: "error",
      });
    }
    ...
    if (modoPrediccion === "inteligente") {
      ...
      setTimeout(() => {
        ...
        if (resultadoCalculado) {
          ...
          const analisis = generarAnalisisAutomatico(
            resultadoCalculado,
            prediccion
          );
          const nuevoHistorial = [
            analisis,
            ...historialPredicciones.slice(0, 9),
          ];
          setHistorialPredicciones(nuevoHistorial);
          guardarHistorialLocal(nuevoHistorial);
        }
  ...
  const calcularForecastCompleto = () => {
    ...
    const totalPedidos = Object.values(prediccionesPorZona).reduce(
      (sum, zona) => sum + zona.pedidos,
      0
    );

    zonas.forEach((zona) => {
      prediccionesPorZona[zona].porcentaje =
        totalPedidos > 0
          ? Math.round((prediccionesPorZona[zona].pedidos / totalPedidos) * 100)
          : 0;
    });
    ...
    const confianza = Math.min(
      95,
      70 + factoresReales.total_pedidos_analizados / 20
    );
  ...
  const generarAnalisisAutomatico = (resultado, prediccion) => {
    ...
    let analisis = {
      titulo: `Predicción del ${fecha.toLocaleDateString("es-ES")} - ${
        prediccion.tipoCliente
      }`,
      fecha: prediccion.fecha,
      tipoCliente: prediccion.tipoCliente,
    ...
    analisis.resumen = `Se esperan ${resultado.totalPedidos} pedidos para el ${
      diasSemana[diaSemana]
    } ${fecha.toLocaleDateString("es-ES")} con ${
      resultado.confianza
    }% de confianza.`;
  ...
  const getTituloModo = () => {
    switch (modoPrediccion) {
      case "inteligente":
        return "Predictor Inteligente";
      case "clasico":
        return "Predictor Clásico";
      default:
        return "Predictor";
    }
  };
        ...
        <Box sx={{ display: "flex", alignItems: "center", gap: 2, mb: 2 }}>
          ...
          <Box>
            ...
            >
              ...
              Análisis predictivo avanzado con calibración dinámica e intervalos
              de confianza
            </Typography>
          </Box>
        </Box>
        ...
        >
          <CardContent sx={{ p: 2 }}>
            <Box sx={{ display: "flex", gap: 1 }}>
              <Button
                variant={
                  modoPrediccion === "inteligente" ? "contained" : "outlined"
                }
              ...
              <Button
                variant={
                  modoPrediccion === "clasico" ? "contained" : "outlined"
                }
                startIcon={<Analytics />}
                onClick={() => setModoPrediccion("clasico")}
                sx={{
                  bgcolor:
                    modoPrediccion === "clasico" ? "#3b82f6" : "transparent",
                  color:
                    modoPrediccion === "clasico" ? "white" : "text.secondary",
                  "&:hover": {
                    bgcolor:
                      modoPrediccion === "clasico"
                        ? "#2563eb"
                        : theme.palette.mode === "dark"
                        ? "rgba(255,255,255,0.05)"
                        : "#f1f5f9",
          ...
          >
            <CardContent sx={{ p: 2 }}>
              <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                ...
                >
                  Predictor Inteligente: Incluye análisis VIP, variables
                  exógenas, calibración dinámica y efectividad estimada
                </Typography>
              </Box>
            </CardContent>
          </Card>
          ...
          >
            <CardContent sx={{ p: 2 }}>
              <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                ...
                >
                  Análisis basado en {factoresReales.total_pedidos_analizados}{" "}
                  pedidos reales de Aguas Ancud (
                  {factoresReales.periodo_analisis?.fecha_inicio || "N/A"} a{" "}
                  {factoresReales.periodo_analisis?.fecha_fin || "N/A"})
                </Typography>
              </Box>
            </CardContent>
          </Card>
              ...
              <Stack spacing={3}>
                <Box sx={{ mb: 2 }}>
                  ...
                  {rangoDatos.fechaMinima && rangoDatos.fechaMaxima && (
                    ...
                    >
                      ...
                      >
                        📅 Datos disponibles: {rangoDatos.fechaMinima} a{" "}
                        {rangoDatos.fechaMaxima}
                      </Typography>
                    </Tooltip>
                  )}
                  {rangoDatos.ultimaActualizacion && (
                    ...
                    >
                      ...
                      {new Date(rangoDatos.ultimaActualizacion).toLocaleString(
                        "es-ES"
                      )}
                    </Typography>
                  )}
                ...
                <Button
                  ...
                  sx={{
                    py: 1.5,
                    bgcolor:
                      modoPrediccion === "inteligente" ? "#8b5cf6" : "#3b82f6",
                    "&:hover": {
                      bgcolor:
                        modoPrediccion === "inteligente"
                          ? "#7c3aed"
            ...
            <CardContent sx={{ p: 3 }}>
              ...
              {prediccionInteligente ? (
                <Box>
                  ...
                  >
                    <CardContent sx={{ p: 3 }}>
                      ...
                      >
                        <Avatar
                          sx={{ bgcolor: "#0ea5e9", width: 48, height: 48 }}
                        >
                        ...
                        <Box>
                          ...
                          <Typography variant="body2" color="text.secondary">
                            Rango de confianza:{" "}
                            {prediccionInteligente.rango_confianza[0]} -{" "}
                            {prediccionInteligente.rango_confianza[1]}
                          </Typography>
                        </Box>
                        <Box sx={{ ml: "auto" }}>
                          <Chip
                            ...
                            sx={{
                              bgcolor: getColorConfianza(
                                prediccionInteligente.nivel_confianza
                              ),
                              color: "white",
                              fontWeight: 600,
                            }}
                      ...
                      <Card
                        sx={{
                          bgcolor: prediccionInteligente.es_anomalia
                            ...
                            : "#f0f9ff",
                          border: `1px solid ${
                            prediccionInteligente.es_anomalia
                              ? "#ef4444"
                              : "#0ea5e9"
                          }`,
                          borderRadius: 3,
                            ...
                            >
                              ...
                              <Stack spacing={1}>
                                ...
                                >
                                  ...
                                  >
                                    {detallesCalculo.totalPedidosHistoricos}{" "}
                                    pedidos
                                  </Typography>
                                </Box>
                                <Box
                            ...
                            >
                              ...
                              <Stack spacing={1}>
                                ...
                                >
                                  ...
                                  >
                                    {detallesCalculo.promedioGeneral?.toFixed(
                                      2
                                    ) || "—"}{" "}
                                    pedidos/día
                                  </Typography>
                                ...
                                >
                                  ...
                                  >
                                    {detallesCalculo.promedioMovil7?.toFixed(
                                      2
                                    ) || "—"}{" "}
                                    pedidos/día
                                  </Typography>
                              ...
                              <Grid container spacing={2}>
                                ...
                                <Grid item xs={12} md={3}>
                                  ...
                                  >
                                    ...
                                    >
                                      {detallesCalculo.promedioDiaSemanaObjetivo?.toFixed(
                                        1
                                      ) || "—"}
                                    </Typography>
                                    <Typography
                                ...
                                <Grid item xs={12} md={3}>
                                  ...
                                  >
                                    ...
                                    >
                                      {detallesCalculo.tendenciaLineal?.toFixed(
                                        1
                                      ) || "—"}
                                    </Typography>
                                    <Typography
                          ...
                          {detallesCalculo.prediccionEnsemble && (
                            <Grid item xs={12}>
                              ...
                              >
                                ...
                                >
                                  Predicción = (Promedio Móvil ×{" "}
                                  {detallesCalculo.pesosEnsemble?.promedioMovil}
                                  ) + (Día Semana ×{" "}
                                  {detallesCalculo.pesosEnsemble?.diaSemana}) +
                                  (Tendencia ×{" "}
                                  {detallesCalculo.pesosEnsemble?.tendencia})
                                </Typography>
                                <Typography
                                  variant="body2"
                                  sx={{
                                ...
                                >
                                  = (
                                  {detallesCalculo.promedioMovil7?.toFixed(2)} ×{" "}
                                  {detallesCalculo.pesosEnsemble?.promedioMovil}
                                  ) + (
                                  {detallesCalculo.promedioDiaSemanaObjetivo?.toFixed(
                                    2
                                  ...
                                  × {detallesCalculo.pesosEnsemble?.diaSemana})
                                  + (
                                  {detallesCalculo.tendenciaLineal?.toFixed(2)}{" "}
                                  × {detallesCalculo.pesosEnsemble?.tendencia})
                                  ={" "}
                                  <strong>
                                    {detallesCalculo.prediccionEnsemble}
                    ...
                    >
                      <CardContent sx={{ p: 3 }}>
                        ...
                        >
                          ...
                          >
                            📊 <strong>Análisis Híbrido:</strong> Basado en{" "}
                            {prediccionInteligente.prediccion} pedidos esperados
                            ×{" "}
                            {
                              prediccionInteligente.prediccion_bidones
                                .promedio_por_pedido
                            }{" "}
                            bidones promedio por pedido
                          </Typography>
                        </Box>
                      </CardContent>
                    </Card>
                  ...
                  {/* Análisis VIP */}
                  {prediccionInteligente.analisis_vip &&
                    prediccionInteligente.analisis_vip.total_vip > 0 && (
                      <Card
                        sx={{
                          mb: 3,
                          bgcolor: "#fef3c7",
                        ...
                        <CardContent sx={{ p: 3 }}>
                          ...
                          {prediccionInteligente.analisis_vip
                            ...
                            <Box>
                              ...
                              <Stack spacing={1}>
                                {prediccionInteligente.analisis_vip.clientes_destacados.map(
                                  (cliente, idx) => (
                                    <Box
                                      key={idx}
                                      sx={{
                                    ...
                                    >
                                      ...
                                      >
                                        {cliente.usuario} -{" "}
                                        {Math.round(
                                          cliente.probabilidad_pedido * 100
                                        )}
                                        % probabilidad
                                      </Typography>
                                      <Typography
                  ...
                  {/* Recomendaciones */}
                  {prediccionInteligente.recomendaciones &&
                    prediccionInteligente.recomendaciones.length > 0 && (
                      <Card
                        sx={{
                          mb: 3,
                          bgcolor: "#f0f9ff",
                      ...
                      >
                        <CardContent sx={{ p: 3 }}>
                          ...
                          <Stack spacing={1}>
                            {prediccionInteligente.recomendaciones.map(
                              (rec, idx) => (
                                <Box
                                  key={idx}
                                  sx={{
                      ...
                      <Stack spacing={2}>
                        <Box>
                          ...
                          <Grid container spacing={1}>
                            <Grid item xs={6}>
                              <Chip
                                label={`Base: ${prediccionInteligente.factores.base}`}
                                size="small"
                              />
                            </Grid>
                            <Grid item xs={6}>
                              <Chip
                                label={`Factor tipo: ${prediccionInteligente.factores.factor_tipo}`}
                                size="small"
                              />
                            </Grid>
                          </Grid>
                          ...
                          <Box>
                            ...
                            <Grid container spacing={1}>
                              <Grid item xs={4}>
                                ...
                                >
                                  Media:{" "}
                                  {prediccionInteligente.estadisticas_base.media?.toFixed(
                                    1
                                  ) || "—"}
                                </Typography>
                              </Grid>
                              <Grid item xs={4}>
                                ...
                                >
                                  Mediana:{" "}
                                  {prediccionInteligente.estadisticas_base.mediana?.toFixed(
                                    1
                                  ) || "—"}
                                </Typography>
                              </Grid>
                              <Grid item xs={4}>
                                ...
                                >
                                  Desv:{" "}
                                  {prediccionInteligente.estadisticas_base.desviacion?.toFixed(
                                    1
                                  ) || "—"}
                                </Typography>
                              </Grid>
                ...
                >
                  ...
                  <Typography
                    variant="h6"
                    sx={{ mb: 1, textAlign: "center", color: "text.primary" }}
                  >
                    Predictor Inteligente
                  </Typography>
                  <Typography
                  ...
                  >
                    Completa los parámetros y genera una predicción inteligente
                    con intervalos de confianza y detección de anomalías
                  </Typography>
                </Box>
              )}
            </CardContent>
      ...
      <Box sx={{ mt: 4 }}>
        ...
        {mostrarTracking && (
          <Grid container spacing={3}>
            ...
            {/* Recomendaciones */}
            {trackingReporte &&
              trackingReporte.recomendaciones &&
              trackingReporte.recomendaciones.length > 0 && (
                <Grid item xs={12}>
                  <Card
                    sx={{
                      bgcolor:

...
import { Box, Typography, FormControl, Select, MenuItem, Card, CardContent, useTheme, Grid, Tooltip, Chip } from '@mui/material';
...
function obtenerPedidosCliente(pedidos, address, user, phone) {
  ...
  return pedidos.filter(p => {
    ...
    const dirPedido = (p.dire || p.direccion || p.customer?.address || '').trim().toLowerCase();
    const matchDireccion = addressLower && dirPedido && dirPedido === addressLower;
    
    // Coincidencia por email/usuario
    const emailPedido = (p.usuario || p.customer?.email || p.customer?.name || '').trim().toLowerCase();
    const matchEmail = userLower && emailPedido && emailPedido === userLower;
    
    // Coincidencia por teléfono
    const telPedido = (p.telefonou || p.phone || p.customer?.phone || '').trim().toLowerCase();
...
function obtenerMetodoPagoUltimoPedido(pedidosCliente) {
  ...
  const metodoPago = ultimoPedido.metodopago || ultimoPedido.paymentMethod || null;
  ...
  if (metodoLower.includes('tarjeta') || metodoLower.includes('webpay')) return 'Tarjeta';
...
function ZoomAwareCircles({ mapData, pedidos, circleRefs }) {
  ...
  return mapData.map((point, index) => {
    ...
    if (!point || !point.lat || !point.lng || isNaN(point.lat) || isNaN(point.lng)) {
      return null;
    }
          ...
          <div style={{
            ...
            <div style={{
              ...
              {(() => {
                ...
                const pedidosCliente = obtenerPedidosCliente(pedidos, point.address, point.user, point.phone);
                
                // Calcular valores
                const ticketPromedio = calcularTicketPromedio(pedidosCliente);
                const totalGastado = calcularTotalGastado(pedidosCliente);
                const promedioDias = calcularPromedioDiasEntrePedidos(pedidosCliente);
                const metodoPago = obtenerMetodoPagoUltimoPedido(pedidosCliente);
                ...
                if (pedidosCliente.length > 0) {
                  ...
                  const ultimoPedido = pedidosOrdenados[0];
                  if (ultimoPedido) {
                    const fechaStr = ultimoPedido.fecha || ultimoPedido.createdAt || ultimoPedido.deliveryDate;
                    const fecha = parseFecha(fechaStr);
                    if (fecha) {
                      fechaUltimoPedido = fecha.toLocaleDateString('es-ES', { 
                        day: '2-digit', 
                        month: '2-digit', 
                        year: 'numeric' 
                      }).replace(/\//g, '-');
                  ...
                  <>
                    ...
                    <div style={{
                      ...
                      <div style={{
                        ...
                        {promedioDias !== null ? `${promedioDias} días` : 'N/A'}
                      </div>
                    </div>
...
export default function MapaCalor() {
  ...
  const fetchHeatmapData = async () => {
    try {
      ...
      if (filterPeriod > 6) {
        ...
        console.log('Solicitando datos sin filtro:', url);
        
        const response = await fetch(url);
        if (response.ok) {
          const data = await response.json();
          console.log('Datos del heatmap recibidos:', data.length, 'puntos');
          
          allMapPoints = data
            .filter(point => point && point.lat && point.lon && !isNaN(point.lat) && !isNaN(point.lon))
            .map((point, index) => ({
        ...
        for (let i = 0; i < filterPeriod; i++) {
          ...
          monthPromises.push(
            fetch(url)
              ...
              .catch(error => {
                console.error(`Error obteniendo datos del mes ${mes}/${anio}:`, error);
                return [];
              })
          );
        }
        ...
        console.log(`✅ ${allMapPoints.length} puntos válidos cargados para período de ${filterPeriod} meses`);
      }
  ...
  const cargarPedidos = async () => {
    try {
      ...
      setPedidos(data || []);
      console.log('Pedidos cargados para cálculo de popup:', data?.length || 0, 'registros');
    } catch (error) {
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} sm={6} md={3}>
              <Card sx={{ 
                ...
                <CardContent sx={{ p: 3 }}>
                  <Tooltip title="Zona con mayor concentración de pedidos en el período seleccionado. Click para ver en el mapa." placement="top" arrow>
                    <Typography 
                      variant="h4" 
                      onClick={() => {
                        if (!mapData || mapData.length === 0 || !mapRef.current) return;
                        ...
                        const openPopupOnce = () => {
                          ...
                          setTimeout(() => {
                            ...
                            if (circleRef) {
                              ...
                              } else if (circleRef.reactComponent?.contextValue?.leafletElement) {
                                leafletElement = circleRef.reactComponent.contextValue.leafletElement;
                              } else if (circleRef.reactComponent?.leafletElement) {
                                leafletElement = circleRef.reactComponent.leafletElement;
                              ...
                              if (leafletElement && typeof leafletElement.openPopup === 'function') {
                    ...
                    >
                      {(() => {
                        ...
                        return maxPoint ? (maxPoint.address || '—').substring(0, 20) + (maxPoint.address && maxPoint.address.length > 20 ? '...' : '') : '—';
                      })()}
                    </Typography>
                  </Tooltip>
                  <Typography variant="body1" sx={{ 
            ...
            <Grid item xs={12} sm={6} md={3}>
              <Card sx={{ 
                ...
                <CardContent sx={{ p: 3 }}>
                  <Tooltip title="Total de pedidos analizados en el período seleccionado" placement="top" arrow>
                    <Typography variant="h4" sx={{ 
                      fontWeight: 700, 
                      color: 'text.primary', 
                      cursor: 'pointer',
            ...
            <Grid item xs={12} sm={6} md={3}>
              <Card sx={{ 
                ...
                <CardContent sx={{ p: 3 }}>
                  <Tooltip title="Valor promedio por pedido en el período seleccionado" placement="top" arrow>
                    <Typography variant="h4" sx={{ 
                      fontWeight: 700, 
                      color: 'text.primary', 
                      cursor: 'pointer',
                      ...
                      {(() => {
                        if (!mapData || mapData.length === 0) return '—';
                        const totalSpent = mapData.reduce((sum, point) => sum + parseInt(point.total_spent || 0), 0);
                        const promedio = mapData.length > 0 ? totalSpent / mapData.length : 0;
                        return promedio > 0 ? `$${Math.round(promedio).toLocaleString('es-CL')}` : '—';
                      })()}
                    </Typography>
                  </Tooltip>
                  <Typography variant="body1" sx={{ 
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                  <Chip 
                    sx={{ 
                      ...
                      borderRadius: '50%', 
                      bgcolor: '#00cc00',
                      border: `2px solid ${theme.palette.mode === 'dark' ? 'rgba(255,255,255,0.2)' : 'white'}`,
                  ...
                  {(() => {
                    ...
                    mapData.forEach(point => {
                      const zona = point.address ? 
                        (point.address.toLowerCase().includes('sur') || point.address.toLowerCase().includes('poniente') ? 'sur-poniente' :
                         point.address.toLowerCase().includes('norte') || point.address.toLowerCase().includes('oriente') ? 'norte-oriente' :
                         'centro') : 'desconocida';
                      puntosPorZona[zona] = (puntosPorZona[zona] || 0) + 1;
                    });
                    const zonaMayor = Object.keys(puntosPorZona).reduce((a, b) => 
                      puntosPorZona[a] > puntosPorZona[b] ? a : b
                    );
                    const zonaMayorNombre = zonaMayor === 'sur-poniente' ? 'sector sur-poniente' :
                                           zonaMayor === 'norte-oriente' ? 'sector norte-oriente' :
                                           zonaMayor === 'centro' ? 'sector centro' : 'zona principal';

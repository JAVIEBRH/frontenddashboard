...
export default function Pedidos({ refreshTrigger = 0 }) {
  ...
  const filteredPedidos = pedidos.filter((p) => {
    ...
    const texto = `${p.fecha || p.createdAt || ""} ${
      p.dire || p.customer?.address || ""
    } ${p.usuario || p.customer?.name || ""} ${p.precio || p.price || ""} ${
      p.status || ""
    }`.toLowerCase();
  ...
  const pedidosPagina = filteredPedidos.slice(
  ...
  const ventasTotales = pedidosFechaSeleccionadaCompletos.reduce(
    (sum, p) => sum + (Number(p.precio) || 0),
    0
  );
  const countEfectivo = pedidosFechaSeleccionadaCompletos.filter((p) =>
    (p.metodopago || "").toLowerCase().includes("efectivo")
  ).length;
  const countTransferencia = pedidosFechaSeleccionadaCompletos.filter((p) =>
    (p.metodopago || "").toLowerCase().includes("transfer")
  ).length;
  const countTarjeta = pedidosFechaSeleccionadaCompletos.filter((p) =>
    (p.metodopago || "").toLowerCase().includes("tarjeta")
  ).length;
  ...
  const bidonesFechaSeleccionada = pedidosFechaSeleccionadaData.reduce(
    (sum, p) => {
      let cantidad = 1;

      // Nuevo esquema: usar products array
      if (p.products && Array.isArray(p.products)) {
        cantidad = p.products.reduce(
  ...
  const agruparClientesPorMetodoPago = (metodoPago) => {
    ...
    pedidosFiltrados.forEach((pedido) => {
      const clienteId =
        ...
        "Sin identificar";
      const nombreCliente =
        pedido.usuario || pedido.customer?.name || "Sin nombre";
      const direccion =
        pedido.dire || pedido.customer?.address || "Sin dirección";
                ...
                <IconButton
                  onClick={() => {
                    if (metodoPago.toLowerCase().includes("efectivo"))
                      setShowEfectivoTable(false);
                    else if (metodoPago.toLowerCase().includes("transfer"))
                      setShowTransferenciaTable(false);
                    else if (metodoPago.toLowerCase().includes("tarjeta"))
                      setShowTarjetaTable(false);
                  }}
                  sx={{
        ...
        <CardContent sx={{ p: 3 }}>
          ...
          <Box sx={{ display: "flex", gap: 3, mb: 3, flexWrap: "wrap" }}>
            ...
            >
              <CardContent sx={{ p: 3 }}>
                <Tooltip
                  title={`Suma total de ventas del ${new Date(
                    selectedDate
            ...
            <Box sx={{ position: "relative" }}>
              <Card
                sx={{
                  minWidth: 180,
                  bgcolor: showEfectivoTable
                    ? theme.palette.primary.main
                    : "background.paper",
                  boxShadow: showEfectivoTable
                    ? theme.shadows[4]
                    : theme.shadows[1],
                  borderRadius: 3,
                  border: `1px solid ${
              ...
              >
                <CardContent sx={{ p: 3, position: "relative" }}>
                  <Tooltip
                    title={`Pedidos pagados en efectivo el ${new Date(
                      selectedDate
                    ).toLocaleDateString("es-ES")}. Click para ver detalles.`}
                    placement="top"
                    arrow
                  >
                    <Typography
              ...
              <TablaClientesPorMetodoPago
                show={showEfectivoTable}
                metodoPago="efectivo"
                titulo="Efectivo"
              />
            </Box>
            <Box sx={{ position: "relative" }}>
              <Card
                sx={{
                  minWidth: 180,
                  bgcolor: showTransferenciaTable
                    ? theme.palette.primary.main
                    : "background.paper",
                  boxShadow: showTransferenciaTable
                    ? theme.shadows[4]
                    : theme.shadows[1],
                  borderRadius: 3,
                  border: `1px solid ${
                    showTransferenciaTable
                      ? theme.palette.primary.main
                      : theme.palette.divider
                  }`,
                  cursor: "pointer",
                ...
                onClick={() =>
                  setShowTransferenciaTable(!showTransferenciaTable)
                }
              >
                <CardContent sx={{ p: 3, position: "relative" }}>
                  <Tooltip
                    title={`Pedidos pagados por transferencia el ${new Date(
                      selectedDate
                    ).toLocaleDateString("es-ES")}. Click para ver detalles.`}
                    placement="top"
                    arrow
                    ...
                    <Add
                      sx={{
                        color: showTransferenciaTable
                          ...
                          : "text.secondary",
                        fontSize: 20,
                        transform: showTransferenciaTable
                          ? "rotate(45deg)"
                          : "rotate(0deg)",
                        transition:
                          "all 0.5s cubic-bezier(0.25, 0.46, 0.45, 0.94)",
            ...
            <Box sx={{ position: "relative" }}>
              <Card
                sx={{
                  minWidth: 180,
                  bgcolor: showTarjetaTable
                    ? theme.palette.primary.main
                    : "background.paper",
                  boxShadow: showTarjetaTable
                    ? theme.shadows[4]
                    : theme.shadows[1],
                  borderRadius: 3,
                  border: `1px solid ${
              ...
              >
                <CardContent sx={{ p: 3, position: "relative" }}>
                  <Tooltip
                    title={`Pedidos pagados con tarjeta el ${new Date(
                      selectedDate
                    ).toLocaleDateString("es-ES")}. Click para ver detalles.`}
                    placement="top"
                    arrow
            ...
            >
              <CardContent sx={{ p: 3 }}>
                ...
                >
                  ...
                  <Tooltip
                    title={`${pedidosFechaSeleccionada} pedidos realizados el ${new Date(
                      selectedDate
                    ).toLocaleDateString("es-ES")}`}
                    placement="top"
                    arrow
            ...
            >
              <CardContent sx={{ p: 3 }}>
                ...
                >
                  ...
                  <Tooltip
                    title={`${bidonesFechaSeleccionada} bidones vendidos el ${new Date(
                      selectedDate
                    ).toLocaleDateString(
                      "es-ES"
                    )} | Total histórico: ${bidonesVendidos} bidones`}
                    placement="top"
                    arrow
                  >
                    <Typography
              ...
              <TableBody>
                {pedidosPagina.map((p, i) => (
                  ...
                  >
                    <TableCell>
                      ...
                      >
                        <Avatar sx={{ bgcolor: "#3b82f6" }}>
                          {(p.usuario || p.customer?.name || "?").charAt(0)}
                        </Avatar>
                        <Box>
                          <Typography
                            variant="body1"
                    ...
                    >
                      ${(p.precio || p.price || 0).toLocaleString()}
                    </TableCell>
                    <TableCell
                      sx={{
                        color: "text.primary",
